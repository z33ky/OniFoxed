...
{
	Uint16 ramp[256];
	SDL_CalculateGammaRamp(inGamma, ramp);
	...
	int depth)
{
	...
	success= SDL_SetWindowDisplayMode(ONgPlatformData.gameWindow, &current_display_mode) == 0;
	UUmAssert(success);
	...
	success= SDL_SetWindowDisplayMode(ONgPlatformData.gameWindow, &desired_display_mode) == 0;
